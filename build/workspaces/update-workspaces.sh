...
if [[ -z "$premake" && ! -x "$(pwd)/premake5" ]]; then
    echo "Downloading premake from github"
    if [ "$(uname -s)" != "Darwin" ]; then
        wget "https://github.com/premake/premake-core/releases/download/v5.0.0-alpha7/premake-5.0.0-alpha7-linux.tar.gz" -O- | \
            tar xz
    else
        curl -L -o- "https://github.com/premake/premake-core/releases/download/v5.0.0-alpha7/premake-5.0.0-alpha7-macosx.tar.gz" | \
            tar xz
    fi
fi
...
$premake --outpath="../workspaces/gcc/" ${premake_args} gmake || die "Premake failed"

# Also generate xcode workspaces if on OS X
if [ "`uname -s`" = "Darwin" ]; then
  $premake --outpath="../workspaces/xcode3" ${premake_args} xcode3 || die "Premake failed"
  $premake --outpath="../workspaces/xcode4" ${premake_args} xcode4 || die "Premake failed"
fi
